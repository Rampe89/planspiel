...
function renderGameAvatar(){
  ...
  el("avatarMoodLabel").textContent =
    ...
    mood === "sad" ? "traurig" : "neutral";

  el("avatarMoodText").textContent =
    mood === "happy"
      ? "Deine Lage ist gerade ziemlich stabil."
      : mood === "sad"
      ? "Deine Figur merkt: Es läuft gerade eher schwierig."
...
function renderReceipt(g){
  ...
  const lines = [
    ["Einkommen", r.income],
    ["Miete", -r.rent],
    ["Nebenkosten", -r.utilities],
    ["Internet", -r.internet],
    ["Handy", -r.phone],
    ["Familie", -r.family],
    ["Essen/Trinken", -r.food],
    ["Freizeit", -r.fun],
    ["Shopping", -r.shop],
    ["Abos", -r.subs],
    ["Mobilität", -r.mobility],
    ["Versicherungen", -r.insurance],
    ["Kreditrate", -r.loan]
  ];
  ...
  const body = lines.map(([label,val]) => `
    <div class="receiptLine">
      <span>${label}</span>
      <span>${val > 0 ? "+" : ""}${formatEUR(val)}</span>
    </div>
  `).join("") + `
    <div class="receiptLine total">
      <strong>Ergebnis nach Fixkosten</strong>
      <strong>${formatEUR(r.afterFixed)}</strong>
    </div>
    <div class="receiptLine">
      <span>Sparen / ETF / Unterkonten</span>
      <span>${formatEUR(-r.planTotal)}</span>
    </div>
    <div class="receiptLine total">
      <strong>Zwischenstand vor Entscheidung/Event</strong>
      <strong>${formatEUR(r.afterPlan)}</strong>
    </div>
  `;

  el("receiptBody").innerHTML = body;
  el("receiptFoot").textContent =
    "Das zeigt grob, wie ein echter Monat aussehen kann: Erst kommen feste Kosten, dann bleibt der Rest.";
}
...
function renderWarnings(g){
  ...
  if(g.balance >= 0){
    lamps.push({ cls:"good", text:"● Konto im Plus" });
  } else if(g.balance > g.dispoFloor){
    lamps.push({ cls:"warn", text:"● Konto im Minus" });
  } else {
    lamps.push({ cls:"bad", text:"● Dispo-Limit erreicht" });
  }

  if(g.buckets.cash >= 1000){
    lamps.push({ cls:"good", text:"● Notgroschen-Ziel erreicht" });
  } else if(g.buckets.cash >= 500){
    lamps.push({ cls:"warn", text:"● Rücklage wächst" });
  } else {
    lamps.push({ cls:"bad", text:"● Kaum Notgroschen" });
  }

  const planned = g.plan.cash + g.plan.etf + g.buckets.subs.reduce((s,b)=>s + (b.plan||0), 0);
  if(planned > 0.35 * g.income){
    lamps.push({ cls:"warn", text:"● Sparplan evtl. zu hoch" });
  }

  if(g.variable.subs >= 35){
    lamps.push({ cls:"warn", text:"● Viele Abos" });
  }

  if(g.redMonths >= 2){
    lamps.push({ cls:"bad", text:"● Mehrere Minus-Monate" });
  }

  if(g.loan.active){
    lamps.push({ cls:"warn", text:"● Laufender Kredit" });
  }
...
function renderStats(g){
  ...
  el("gBalanceHint").textContent =
    g.balance < g.dispoFloor ? "Kontosperre (Limit erreicht)" :
    (g.balance < 0 ? "Im Minus" : "OK");
...
function drawLineChart(canvas, values){
  ...
  for(let i=1;i<values.length;i++){
    ctx.lineTo(pad + i*stepX, normY(values[i]));
  }
  ctx.stroke();
}
...
function impactPills({money=0, stability=0, comfort=0, social=0}){
  ...
  const pills = [];
  pills.push(mk(money>0?"good":money<0?"bad":"neu", `${money>0?"+":""}${formatEUR(money)}`));
  pills.push(mk(stability>0?"good":stability<0?"bad":"neu", `Stabil ${stability>0?"+":""}${stability}`));
  pills.push(mk(comfort>0?"good":comfort<0?"bad":"neu", `Komfort ${comfort>0?"+":""}${comfort}`));
  pills.push(mk(social>0?"good":social<0?"bad":"neu", `Soz. ${social>0?"+":""}${social}`));
  return pills;
}
        ...
        options: [
          {
            ...
            impacts: impactPills({money:-120, stability:-1, comfort:-1, social:-1}),
            onPick: ()=>{
              applyOption(g, {label:"Reparieren", meta:"Billiger.", money:-120, stability:-1, comfort:-1, social:-1}, "Ereignis");
              finalizeMonth(g);
            }
          },
          {
            ...
            impacts: impactPills({money:-420, stability:-2, comfort:+2, social:+2}),
            onPick: ()=>{
              applyOption(g, {label:"Neu kaufen", meta:"Teurer.", money:-420, stability:-2, comfort:+2, social:+2}, "Ereignis");
              finalizeMonth(g);
            }
          }
        ]
        ...
        options: [
          {
            ...
            onPick: ()=>{
              applyOption(g, {label:"Behalten", meta:"Mehr Luft.", money:+120, stability:+2, comfort:+1, social:+1}, "Ereignis");
              finalizeMonth(g);
            }
          },
          {
            ...
            onPick: ()=>{
              applyOption(g, {label:"Teil sparen", meta:"Du legst direkt was weg.", money:+120, stability:+3, comfort:0, social:0}, "Ereignis");
              g.buckets.cash += 80;
              g.balance -= 80;
    ...
    options: [
      {
        label: d.a.label,
        meta: d.a.meta,
        impacts: impactPills(d.a),
        onPick: ()=>{
          applyOption(g, d.a, "Entscheidung");
          continueAfterDecision(g);
        }
      },
      {
        label: d.b.label,
        meta: d.b.meta,
        impacts: impactPills(d.b),
        onPick: ()=>{
          applyOption(g, d.b, "Entscheidung");
          continueAfterDecision(g);
        }
      }
    ]
...
function showGlossary(){
  el("glossaryContent").innerHTML = `
    <div class="gItem">
      ...
      <div class="gText">Rücklage für Notfälle wie kaputte Waschmaschine, Handy oder Jobverlust.</div>
      <div class="gSmall">Idee: Erst Rücklage, dann größere Extras.</div>
    </div>
    <div class="gItem">
      <div class="gTitle">ETF</div>
      <div class="gText">Eine Form von langfristigem Investieren. Der Wert kann steigen oder fallen.</div>
      <div class="gSmall">Im Spiel siehst du diese Schwankung jeden Monat.</div>
    </div>
    <div class="gItem">
      <div class="gTitle">Dispo</div>
      <div class="gText">Du bist im Minus auf dem Konto. Das ist oft teuer.</div>
      <div class="gSmall">Im Spiel gibt es deshalb ein Dispo-Limit.</div>
    </div>
    <div class="gItem">
      <div class="gTitle">Fixkosten</div>
      <div class="gText">Kosten, die fast jeden Monat wiederkommen, zum Beispiel Miete oder Internet.</div>
      <div class="gSmall">Die sollte man zuerst im Blick haben.</div>
    </div>
  `;
...
function bindInterviewLive(){
  const ids = [
    "path","field","living","family","style",
    "lifeFood","lifeFun","lifeShop","lifeSubs","lifeMobility"
  ];
